...
const HTTPS="https://";
const END_POINTS=["in1","sg1","us1","eu1"];
...
var CleverTapAPI = function (CleverTapAccountId, CleverTapAccountPasscode,CleverTapEndPoint) {
    ...
    this.apiEndpoint=findEndPoint(CleverTapEndPoint);

};

function findEndPoint(CleverTapEndPoint){
    if(CleverTapEndPoint && (END_POINTS.includes(CleverTapEndPoint))){
        this.endpoint=CleverTapEndPoint;
    }else{
        console.log("Please provide a valid API Endpoint");
    }
    return `${HTTPS}${this.endpoint ? (this.endpoint+".") : ""}${API_HOSTNAME}/${API_VERSION}`;
}
